...
"""Adapted from https://github.com/NVIDIA/Megatron-LM/blob/main/megatron/data/data_samplers.py
...
def my_collate_fn(task_list: List[GatoInputBase]):
    """
...
def build_naive_data_loader(ds, rand_i=False):
    """Naive data loader for get only one sample"""
    i = 0
    while True:
        res = [ds[i]]
        yield res
...
def build_pretraining_data_loader(
    args, dataset, consumed_samples, 
    total_samples: Optional[int], eval=False
):
    ...
    if args.dataloader_type == "single":
        batch_sampler = SequentialPretrainingSampler(
        ...
        if eval:
            batch_sampler = SequentialPretrainingSampler(
                total_samples=total_samples,
                consumed_samples=consumed_samples,
                micro_batch_size=args.micro_batch_size * eval,
                data_parallel_rank=0,
                data_parallel_size=1,
            )
        else:
            batch_sampler = RandomPretrainingSampler(
                dataset,
                total_samples=total_samples,
                consumed_samples=consumed_samples,
                micro_batch_size=args.micro_batch_size,
...
class RandomPretrainingSampler:
    ...
    def __iter__(self):
        ...
        if self.data_sharding:
            ...
            random_idx = torch.randperm(bucket_size, generator=g)
            idx_range = start_idx + random_idx[bucket_offset:]
        else:
            full_bucket_size = (
