...
# Adapted from https://github.com/NVIDIA/Megatron-LM/blob/main/megatron/model/gpt_model.py
...
from src.data.input_specs import NLPTaskInput
from src import mpu
...
def get_ltor_masks_and_position_ids(
    data,
    eod_token_id,
    # TODO: remove these input and modify the code calling this function.
    reset_position_ids, # unused argument
    reset_attention_mask, # unused argument
    eod_mask_loss,
):
    ...
    attention_mask = np.tril(np.ones((seq_length, seq_length)))

    # Loss mask.
    loss_mask = np.ones(data.shape, dtype=np.float32)
    if eod_mask_loss:
        loss_mask[data == eod_token_id] = 0.0
    ...
    #                 attention_mask[
    #                     b, 0, (text_shift + i + 1) :, : (text_shift + i + 1)
    #                 ] = 0
    #             # Reset positions.
    #             if reset_position_ids:
...
class GPTDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        num_samples = np.sum(self.indexed_dataset.sizes[documents]) // seq_length
    ...
    def __getitem__(self, idx):
        ...
        res = NLPTaskInput(
            ...
            attention_mask=None,
            loss_mask=loss_mask,
            label=sample[1 : self.seq_length + 1],
            text_seq=tokens,
            # for inference, ignore at training
            text_len=None,
        )
        res.apply(lambda x: x.astype(np.int32) if x.dtype == np.uint16 else x)
