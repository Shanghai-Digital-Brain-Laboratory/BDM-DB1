/*Copy and Adapted from https://github.com/NVIDIA/Megatron-LM/blob/main/megatron/data/helpers.cpp */
...
py::array build_rl_sample_idx(
  const py::array_t<int32_t>& path_lengths_,
  const int32_t transition_num
) {
  auto path_lengths = path_lengths_.unchecked<1>();

  auto num_of_path = path_lengths_.shape(0);
  int64_t num_of_idx = 0;
  for(int32_t i = 0; i < num_of_path; ++i) {
    num_of_idx += path_lengths[i] - 1;
  }

  int32_t* sample_idx = new int32_t[3*num_of_idx];
  int64_t i_sample = 0;
  for(int32_t i = 0; i < num_of_path; ++i) {
    for(int32_t j=0; j < path_lengths[i] - 1; ++j) {
      sample_idx[i_sample * 3] = i;
      sample_idx[i_sample * 3 + 1] = j;
      sample_idx[i_sample * 3 + 2] = min(j + transition_num, path_lengths[i]);
      ++i_sample;
    }
  }
...
delete[] mem;
  ...
  return py::array(std::vector<int64_t>{num_of_idx, 3}, // shape
...
PYBIND11_MODULE(helpers, m) {
    ...
    m.def("build_rl_sample_idx", &build_rl_sample_idx);
}
